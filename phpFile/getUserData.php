...
if (!empty($identifier) && !empty($password)) {
    if (verifyPassword($identifier, $bdd, $password)) {
        $user = $bdd->prepare("SELECT id, name, email, organization, ppLink, role, association_id FROM user WHERE email = ? OR name = ? LIMIT 1");
    ...
    $user = $bdd->prepare("SELECT id, name, email, organization, ppLink, role, association_id FROM user WHERE id = ?");
    $user->execute([$id]);
    echo json_encode($user->fetchAll(PDO::FETCH_ASSOC));
} else {
